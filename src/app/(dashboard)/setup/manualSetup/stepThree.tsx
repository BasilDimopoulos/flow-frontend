              ...
              >
                ...
                {chatPersonalisation.map((setting) => (
                  ...
                  >
                    <input
                      value={setting.id}
                      checked={assistant.messaging_style === setting.id}
                      onChange={() =>
                        onAssistantUpdate({ messaging_style: setting.id })
                      }
                ...
                {howManyMessages.map((setting) => (
                  ...
                  >
                    <input
                      value={setting.id}
                      checked={
                        assistant.number_of_messages !== null &&
                        assistant.number_of_messages != undefined &&
                        assistant.number_of_messages > -1
                      }
                      onChange={() => {
                        if (setting.id === 'no-hard-limit') {
                          onAssistantUpdate({ number_of_messages: -1 })
                        } else if (setting.id === 'hard-limit') {
                          onAssistantUpdate({
                            number_of_messages:
                              assistant.number_of_messages || 1,
                          })
                        ...
                        <div>
                          <div className="mt-2">
                            <input
                              ...
                              value={assistant.number_of_messages || ''}
                              onChange={(e) =>
                                onAssistantUpdate({
                                  number_of_messages: e.target
              ...
              >
                ...
                {conversionSettings.map((setting) => (
                  ...
                  >
                    <input
                      value={setting.id}
                      checked={assistant.conversion?.goal === setting.id}
                      onChange={() =>
                        onAssistantUpdate({
                          conversion: {
                    ...
                    <span className="ml-3 flex flex-col">
                      ...
                      <div>
                        {assistant.conversion?.goal === setting.id && (
                          <div className="mt-2">
                            <input
                              id={`link-${setting.name}`}
                              name={`link-${setting.name}`}
                              type="link"
                              value={assistant?.conversion?.link}
                              onChange={(e) =>
                                onAssistantUpdate({
                                  conversion: {
                                    ...(assistant.conversion || {}),
